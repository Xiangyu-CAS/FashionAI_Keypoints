...
def construct_model(args):
    model = models.CPM_ResNet.pose_estimation(class_num=25, pretrain=True)
    model.cuda()
    print (model)
    return model
...
def train_net(model, args):
	...
	while iters < config.max_iter:
		for i, (input, heatmap) in enumerate(train_loader):
			...
			# loss2 = criterion(heat, heatmap_var)
			# loss3 = criterion(heat, heatmap_var)
			# loss4 = criterion(heat, heatmap_var)
