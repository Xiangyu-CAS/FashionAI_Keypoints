...
class pose_estimation(nn.Module):
    def __init__(self, class_num, pretrain=True):
        ...
        # self.resnet = ResNet(Bottleneck, [3, 4, 23, 3]) #resnet101
        if pretrain == True:
            #self.model_path = '/data/xiaobing.wang/.torch/models/resnet101-5d3b4d8f.pth'
        ...
        self.block_P3 = nn.Sequential(nn.Conv2d(256, 128, 1, 1, 0), nn.ReLU(inplace=True),
        ...
        self.block_P4 = nn.Sequential(nn.Conv2d(256, 128, 1, 1, 0), nn.ReLU(inplace=True),
                                      nn.Conv2d(128, 128, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(128, 128, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(128, 128, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(128, 128, 3, 1, 1), nn.ReLU(inplace=True)
        ...
        self.block_P5 = nn.Sequential(nn.Conv2d(256, 256, 1, 1, 0), nn.ReLU(inplace=True),
                                      nn.Conv2d(256, 256, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(256, 256, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(256, 256, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(256, 128, 3, 1, 1), nn.ReLU(inplace=True)
                                      )
        self.block_P6 = nn.Sequential(nn.Conv2d(256, 256, 1, 1, 0), nn.ReLU(inplace=True),
                                      nn.Conv2d(256, 256, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(256, 256, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(256, 256, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(256, 128, 3, 1, 1), nn.ReLU(inplace=True)
        ...
        self._init_weights(self.block_P3)
        self._init_weights(self.block_P4)
        self._init_weights(self.block_P5)
        self._init_weights(self.block_P6)


        self.predict_P3 = nn.Conv2d(128, 25, 3, 1, 1)
        self.predict_P4 = nn.Conv2d(128, 25, 3, 1, 1)
        self.predict_P5 = nn.Conv2d(128, 25, 3, 1, 1)
        self.predict_P6 = nn.Conv2d(128, 25, 3, 1, 1)

        self.predict =  nn.Conv2d(25 * 4, 25, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        out_P3 = self.block_P3(P3)
        out_P4 = self.block_P4(P4)
        out_P5 = self.block_P5(P5)
        out_P6 = self.block_P6(P6)

        out_P4 = F.upsample(out_P4, scale_factor=2)
        out_P5 = F.upsample(out_P5, scale_factor=4)
        out_P6 = F.upsample(out_P6, scale_factor=8)

        heat_P3 = self.predict_P3(out_P3)
        heat_P4 = self.predict_P4(out_P4)
        heat_P5 = self.predict_P5(out_P5)
        heat_P6 = self.predict_P6(out_P6)

        pred_concat = torch.cat([heat_P3, heat_P4, heat_P5, heat_P6], 1)

        heat = self.predict(pred_concat)

        return heat, heat_P3, heat_P4, heat_P5, heat_P6
