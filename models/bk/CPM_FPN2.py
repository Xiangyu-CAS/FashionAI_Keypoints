...
class pose_estimation(nn.Module):
    def __init__(self, class_num, pretrain=True):
        ...
        self.block_P4_upsample = nn.Sequential(nn.Conv2d(256, 128, 1, 1, 0), nn.ReLU(inplace=True),
        ...
        self.block_P5 = nn.Sequential(nn.Conv2d(256, 128, 1, 1, 0), nn.ReLU(inplace=True),
                                    nn.Conv2d(128, 128, 3, 1, 1), nn.ReLU(inplace=True),
                                    nn.Conv2d(128, 128, 3, 1, 1), nn.ReLU(inplace=True),
                                    nn.Conv2d(128, 128, 3, 1, 1), nn.ReLU(inplace=True),
                                    nn.Conv2d(128, 512, 3, 1, 1), nn.ReLU(inplace=True)
                                    )
        self.block_P6 = nn.Sequential(nn.Conv2d(256, 128, 1, 1, 0), nn.ReLU(inplace=True),
                                    nn.Conv2d(128, 128, 3, 1, 1), nn.ReLU(inplace=True),
                                    nn.Conv2d(128, 128, 3, 1, 1), nn.ReLU(inplace=True),
                                    nn.Conv2d(128, 128, 3, 1, 1), nn.ReLU(inplace=True),
                                    nn.Conv2d(128, 512, 3, 1, 1), nn.ReLU(inplace=True)
                                    )
        self._init_weights(self.block_P4_upsample)
        self._init_weights(self.block_P4)
        self._init_weights(self.block_P5)
        self._init_weights(self.block_P6)

        self.predict_P4_upsample = nn.Conv2d(512, 25, 1, 1, 0)
        self.predict_P4 = nn.Conv2d(512, 25, 1, 1, 0)
        self.predict_P5 = nn.Conv2d(512, 25, 1, 1, 0)
        self.predict_P6 = nn.Conv2d(512, 25, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        p5_out = self.P5_conv2(p5_out)
        p4_out = self.P4_conv2(p4_out)
        p4_out_upsample = F.upsample(p4_out, scale_factor=2)
		
        # p6_out = self.block_P6(p6_out)
        # p5_out = self.block_P5(p5_out)
        # p4_out = self.block_P4(p4_out)
        p4_out_upsample = self.block_P4_upsample(p4_out_upsample)


        # p6_heat = self.predict_P6(p6_out)
        # p6_heat = F.upsample(p6_heat, scale_factor=8)
        # p5_heat = self.predict_P5(p5_out)
        # p5_heat = F.upsample(p5_heat, scale_factor=4)
        # p4_heat = self.predict_P4(p4_out)
        # p4_heat = F.upsample(p4_heat, scale_factor=2)
        p4_heat_upsample = self.predict_P4_upsample(p4_out_upsample)
        
        return p4_heat_upsample #, p4_heat, p5_heat, p6_heat